...
export class AdminService {
  ...
  async getDashboard(now: Date = new Date()): Promise<AdminDashboard> {
    const [totals, revenueByWeek, topProducts, ordersByStatus, lowStock] =
      await Promise.all([
        this.getTotals(),
        this.getRevenueByWeek(now),
        this.getTopProducts(),
  ...
  private async getRevenueByWeek(now: Date): Promise<RevenueByWeek[]> {
    ...
    for (const order of orders) {
      const created = parseStoredDate(order.createdAt);
...
/**
 * Parse a value read back from the DB. SQLite stores dates as a naive UTC
 * datetime string ("YYYY-MM-DD HH:mm:ss.SSS"); `new Date(...)` would otherwise
 * reinterpret that wall-clock as local time and shift it by the offset.
 */
 ...
 * UTC throughout so bucketing is independent of the server's timezone and
 * stays consistent with the UTC timestamps stored by TypeORM.
 */
function isoWeekMonday(date: Date): Date {
  const monday = new Date(
    Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()),
  );
  // getUTCDay(): 0 (Sun) .. 6 (Sat). Map to offset back to Monday.
...
function buildWeekBuckets(now: Date, count: number): WeekBucket[] {
  ...
  for (let i = count - 1; i >= 0; i--) {
    // Step by UTC calendar days so the keys stay aligned with isoWeekMonday(),
    // which is reused to place each order. UTC has no DST, so no day can shift.
    const start = new Date(currentMonday);
    start.setUTCDate(start.getUTCDate() - i * DAYS_PER_WEEK);
