...
### Docker fallback (if the native start won't run)

If `npm run start` fails to build on your machine — most often on **Windows**, where the native
addons (`better-sqlite3`, `esbuild`) won't compile — run the app in containers instead.
Dependencies are installed **inside** a Linux container, so your host toolchain never gets in the
way. **You need:** [Docker Desktop](https://www.docker.com/products/docker-desktop/).

```bash
docker compose up          # builds + installs (first run takes a minute), then starts both
```

Shortcuts: macOS/Linux `./start-docker.sh` · Windows `start-docker.cmd` (double-clickable).

Then open the same URLs as the native path — **http://localhost:9010** (web), API on
**http://localhost:8010** — and sign in with the accounts above.

**Notes**
- **Live edits work.** Your source is mounted into the containers; saving a file hot-reloads the
  app (web HMR and API watch mode), so the agent's changes show up without a rebuild.
- `node_modules` and the SQLite db live in Docker volumes (`deps`, `api-db`), never on your host —
  the db re-seeds on boot and is fully disposable.
- `docker compose down` stops it; `docker compose down -v` also wipes the volumes for a clean
  reset. Re-run `docker compose up --build` after changing the `Dockerfile` or dependencies.
...
From the repo root (note the `./` — a bare `.` is rejected as an invalid source):
...
> If `/plugin` isn't recognized, update Claude Code (`claude --version`, then update to the
> latest). You can sanity-check the marketplace with `/plugin validate ./`.
